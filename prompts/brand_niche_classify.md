...
- `other` — the brand clearly does not fit any bucket above (e.g. apparel, electronics, haircare, makeup, pet, food positioned as food not a supplement).

Classification rules:
- Judge by the brand's PRIMARY product positioning, not a single outlier SKU. If most of the catalog is ingestible wellness, it's `supplement`; if most is topical skin, it's `skincare`; if most is household/home cleaning, it's `cleaning`.
- Key dividing lines: ingestible → `supplement`; applied to the body/skin → `skincare`; used to clean surfaces, laundry, or the home → `cleaning`. A "cleanser" for the face is `skincare`; a cleaner for counters/floors/dishes is `cleaning`.
...
{"niche": "<supplement|skincare|cleaning|other>", "confidence": <number between 0 and 1>, "reasoning": "<one short sentence, under 20 words>"}
